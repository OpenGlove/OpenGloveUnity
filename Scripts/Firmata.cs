 ...
 * Copyright (C) 2014 Modified for OpenGlove
 * 
 * 
 *
...
namespace OpenGlove
{
	...
	public partial class Arduino : ArduinoBase
	{
		...
		private const int I2C_REQUEST 			= 0x76; //
		private const int I2C_REPLY 			= 0x77; //
		...
		public void I2C_Read(int address, bool continuous)
		{
			byte readMode;
			if (continuous) {
				readMode = 0x10;
			...
			command [0] = (byte)START_SYSEX;
			command [1] = (byte)I2C_REQUEST;
			command [2] = (byte)address;
			command [3] = (byte)readMode;
			command [4] = (byte)END_SYSEX;
			_serialPort.Write(command, 0, 5);
		}
		...
		void processInput()
		{	
			UnityEngine.Debug.Log ("processInput");
			...
			while(processed < BATCH_PROCESS_BYTES_LIMIT) 
			{
				...
				{	
					...
					else if (waitForData > 0 && inputData < 128)
					{
						...
						{
							UnityEngine.Debug.Log("Stored Input data=>");
							UnityEngine.Debug.Log(storedInputData[1].ToString());
							//we got everything
